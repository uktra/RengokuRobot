...
import YoneRobot.modules.helper_funcs.fun_strings as fun
from YoneRobot import dispatcher
from YoneRobot.modules.disable import DisableAbleCommandHandler, DisableAbleMessageHandler
...
def bonk(update: Update, context: CallbackContext):
    message = update.effective_message
    name = message.reply_to_message.from_user.first_name if message.reply_to_message else message.from_user.first_name
    reply_video = message.reply_to_message.reply_animation if message.reply_to_message else message.reply_animation
    reply_video(CHEEMS_BONKLOL, caption=f'*Bonks {name}*')
...
__handlers__ = [
    ...
    SHRUG_HANDLER, BLUETEXT_HANDLER, RLG_HANDLER, DECIDE_HANDLER, TABLE_HANDLER, BONK_HANDLER,
